...
def remove_release_number(data: pd.DataFrame, column: str) -> pd.DataFrame:
    """
...
def encode_answer(answer: Union[str, int], encode_letter: bool = True) -> Union[int, str]:
    """
...
def rag(row: pd.Series, query_eng: RetrieverQueryEngine, top_k: int) -> str:
    """
...
def generate_prompt(row: pd.Series, context: str) -> str:
    """
...
def llm_inference(data: pd.DataFrame,
                  model: Union[PhiForCausalLM, PeftModelForCausalLM],
...
def get_results_with_labels(results_df: pd.DataFrame,
                            labels_df: pd.DataFrame) -> Tuple[pd.DataFrame, float]:
    """
